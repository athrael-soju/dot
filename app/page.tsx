        ...
        <>
          ...
          <div className="absolute top-4 right-4 flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-lg px-3 py-2">
            ...
            <button
              ...
              className={`relative w-10 h-5 rounded-full transition-colors ${debugMode ? 'bg-green-500' : 'bg-white/30'
                }`}
            >
              <span
                className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${debugMode ? 'translate-x-5' : 'translate-x-0'
                  }`}
              />
            </button>
          </div>
